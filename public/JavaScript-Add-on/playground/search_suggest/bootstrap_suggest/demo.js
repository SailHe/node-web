(function () {
    ...
    $("#taobao").bsSuggest({
        ...
        effectiveFieldsAlias: {Id: "序号", goodsCode: "商品编码", goodsName: "商品名称", goodsShortName: "商品简称", goodsMnemonicCode: "助记码"},
        //url: 'http://suggest.taobao.com/sug?code=utf-8&extras=1&q=', /*优先从url ajax 请求 json 帮助数据，注意最后一个参数为关键字请求参数*/
        url: 'http://localhost:8090/goods/fuzzyQuery?search=',
        ...
        fnProcessData: function (json) {
            ...
            for (index = 0; index < len; index++) {
                data.value.push({
                    "Id": (index + 1),
                    "goodsCode": json.data[index].goodsCode,
                    "goodsName": json.data[index].goodsName,
                    "goodsShortName": json.data[index].goodsShortName,
                    "goodsMnemonicCode": json.data[index].goodsMnemonicCode,
                });
            }
    ...
    $("#goodsSearchLink").bsSuggest({
        ...
        fnProcessData: function (json) {
            ...
            for (index = 0; index < len; index++) {
                data.value.push({
                    "Id": (index + 1),
                    "goodsId": json.data[index].goodsId,
                    "goodsCode": json.data[index].goodsCode,
                    "goodsName": json.data[index].goodsName,
                    "goodsShortName": json.data[index].goodsShortName,
                    "goodsMnemonicCode": json.data[index].goodsMnemonicCode,
            ...
            console.log('美丽健搜索 API: ', data);
            return data;
        }
    }).on('onDataRequestSuccess', function (e, result) {
        console.log('onDataRequestSuccess: ', result);
    }).on('onSetSelectValue', function (e, keyword, data) {
        linkKey = data.goodsId;
        $('#goodsSkuSearchLink').val(data.goodsId);
        console.log('onSetSelectValue: ', keyword, data);
    ...
    $("#goodsSkuSearchLink").bsSuggest({
        ...
        showHeader: true,       //显示多个字段的表头
        autoDropup: true,       //自动判断菜单向上展开
        effectiveFieldsAlias: {Id: "序号", goodsSkuTaste: "口味", goodsSkuAttrsJson: "容量"},
        url: 'http://localhost:8090/goods/sku/fuzzyQuery?search=',
        // url 获取数据时，对数据的处理，作为 fnGetData 的回调函数
        fnProcessData: function (json) {
            //debugger;
            var index, len, data = {value: []};
            ...
            for (index = 0; index < len; index++) {
                data.value.push({
                    "Id": (index + 1),
                    "goodsSkuTaste": json.data[index].goodsSkuTaste,
                    "goodsSkuAttrsJson": json.data[index].goodsSkuAttrsJson,
                });
            }
            console.log('美丽健搜索 API: ', data);
            return data;
